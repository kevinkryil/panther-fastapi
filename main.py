...
@app.delete("/api/invoice/{id}/delete", response_model=schemas.InvoiceCreate)
...
@app.post("/api/invoice/create", response_model=schemas.Invoice)
def create_invoice(request: schemas.InvoiceCreate, db: Session = Depends(get_db)):
    db_invoice = models.Invoice(product=request.product)
    db.add(db_invoice)
    db.commit()
